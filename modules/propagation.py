...
def propagate_up(ontology_dataclasses_list, gufo_dictionary, graph, nodes_list, input_node, action_code, call,
    ...
    if input_node not in nodes_list["roots"]:
        ...
        if input_node not in list_restrictions:
            perform_rule_actions_types(ontology_dataclasses_list, gufo_dictionary, action_code, list_restrictions)

        for parent_node in list_parent_nodes:
            call = call + 1
            propagate_up(ontology_dataclasses_list, gufo_dictionary, graph, nodes_list, parent_node, action_code, call,
                         list_restrictions)
...
def propagate_down(ontology_dataclasses_list, gufo_dictionary, graph, nodes_list, input_node, action_code, call,
    ...
    if input_node not in nodes_list["leaves"]:
        ...
        # Execute actions if not in restriction list.
        if input_node not in list_restrictions:
            perform_rule_actions_types(ontology_dataclasses_list, gufo_dictionary, list_child_nodes, action_code,
                                       list_restrictions)

        for child_node in list_child_nodes:
            call = call + 1
            propagate_down(ontology_dataclasses_list, gufo_dictionary, graph, nodes_list, child_node, action_code, call,
                           list_restrictions)
