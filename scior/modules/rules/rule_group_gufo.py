...
def execute_gufo_positive_rules(ontology_dataclass_list):
    ...
    for ontology_dataclass in ontology_dataclass_list:
        ...
        if "RigidType" in ontology_dataclass.is_type:
            ...
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "NonRigidType",
                                                   rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "AntiRigidType",
                                                   rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "SemiRigidType",
                                                   rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Role", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Phase", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "RoleMixin", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "PhaseMixin", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Mixin", rule_code)

        # IR03: NonRigidType(x) -> ~RigidType(x)
        if "NonRigidType" in ontology_dataclass.is_type:
            rule_code = "IR03"
            # LOGGER.debug(f"Executing rule {rule_code} for {ontology_dataclass.uri}.")
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "RigidType", rule_code)
        ...
        if "AntiRigidType" in ontology_dataclass.is_type:
            rule_code = "IR10"
            # LOGGER.debug(f"Executing rule {rule_code} for {ontology_dataclass.uri}.")
            m.move_classification_to_is_type_list(ontology_dataclass_list, ontology_dataclass, "NonRigidType",
                                                  rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "SemiRigidType",
                                                   rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Category", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Kind", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "SubKind", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Mixin", rule_code)
        ...
        if "SemiRigidType" in ontology_dataclass.is_type:
            rule_code = "IR12"
            # LOGGER.debug(f"Executing rule {rule_code} for {ontology_dataclass.uri}.")
            m.move_classification_to_is_type_list(ontology_dataclass_list, ontology_dataclass, "Mixin", rule_code)
            m.move_classification_to_is_type_list(ontology_dataclass_list, ontology_dataclass, "NonRigidType",
                                                  rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "AntiRigidType",
                                                   rule_code)
        ...
        if "Sortal" in ontology_dataclass.is_type:
            ...
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "NonSortal", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Category", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "PhaseMixin", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "RoleMixin", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Mixin", rule_code)
        ...
        if "NonSortal" in ontology_dataclass.is_type:
            rule_code = "IR16"
            # LOGGER.debug(f"Executing rule {rule_code} for {ontology_dataclass.uri}.")
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Sortal", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Kind", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Phase", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Role", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "SubKind", rule_code)
        ...
        if "Mixin" in ontology_dataclass.is_type:
            rule_code = "IR27"
            # LOGGER.debug(f"Executing rule {rule_code} for {ontology_dataclass.uri}.")
            m.move_classification_to_is_type_list(ontology_dataclass_list, ontology_dataclass, "NonSortal", rule_code)
            m.move_classification_to_is_type_list(ontology_dataclass_list, ontology_dataclass, "SemiRigidType",
                                                  rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Category", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "PhaseMixin", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "RoleMixin", rule_code)
        ...
        if "Category" in ontology_dataclass.is_type:
            ...
            # LOGGER.debug(f"Executing rule {rule_code} for {ontology_dataclass.uri}.")
            m.move_classification_to_is_type_list(ontology_dataclass_list, ontology_dataclass, "NonSortal", rule_code)
            m.move_classification_to_is_type_list(ontology_dataclass_list, ontology_dataclass, "RigidType", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Kind", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Mixin", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "PhaseMixin", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "RoleMixin", rule_code)
        ...
        if "RoleMixin" in ontology_dataclass.is_type:
            rule_code = "IR25"
            # LOGGER.debug(f"Executing rule {rule_code} for {ontology_dataclass.uri}.")
            m.move_classification_to_is_type_list(ontology_dataclass_list, ontology_dataclass, "AntiRigidType",
                                                  rule_code)
            m.move_classification_to_is_type_list(ontology_dataclass_list, ontology_dataclass, "NonSortal", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Category", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Mixin", rule_code)
        ...
        if "Kind" in ontology_dataclass.is_type:
            ...
            # LOGGER.debug(f"Executing rule {rule_code} for {ontology_dataclass.uri}.")
            m.move_classification_to_is_type_list(ontology_dataclass_list, ontology_dataclass, "RigidType", rule_code)
            m.move_classification_to_is_type_list(ontology_dataclass_list, ontology_dataclass, "Sortal", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Category", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Phase", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "Role", rule_code)
            m.move_classification_to_not_type_list(ontology_dataclass_list, ontology_dataclass, "SubKind", rule_code)
...
def execute_gufo_negative_rules(ontology_dataclass_list):
    ...
    for ontology_dataclass in ontology_dataclass_list:
        ...
        if "Role" in ontology_dataclass.not_type and "Phase" in ontology_dataclass.not_type \
                and "RoleMixin" in ontology_dataclass.not_type and "PhaseMixin" in ontology_dataclass.not_type \
                and "Mixin" in ontology_dataclass.not_type:
            rule_code = "IR11"
            # LOGGER.debug(f"Executing rule {rule_code} for {ontology_dataclass.uri}.")
            m.move_classification_to_is_type_list(ontology_dataclass_list, ontology_dataclass, "RigidType", rule_code)

        # IR09: ~Category(x) ^ ~Kind(x) ^ ~SubKind(x) ^ ~Mixin(x) -> AntiRigidType(x)
        if "Category" in ontology_dataclass.not_type and "Kind" in ontology_dataclass.not_type \
                and "SubKind" in ontology_dataclass.not_type and "Mixin" in ontology_dataclass.not_type:
            rule_code = "IR09"
            # LOGGER.debug(f"Executing rule {rule_code} for {ontology_dataclass.uri}.")
            m.move_classification_to_is_type_list(ontology_dataclass_list, ontology_dataclass, "AntiRigidType",
                                                  rule_code)
        ...
        #           SemiRigidType(x)
        if "Category" in ontology_dataclass.not_type and "Kind" in ontology_dataclass.not_type \
                and "SubKind" in ontology_dataclass.not_type and "Role" in ontology_dataclass.not_type \
                and "Phase" in ontology_dataclass.not_type and "RoleMixin" in ontology_dataclass.not_type \
                and "PhaseMixin" in ontology_dataclass.not_type:
            rule_code = "IR08"
            # LOGGER.debug(f"Executing rule {rule_code} for {ontology_dataclass.uri}.")
            m.move_classification_to_is_type_list(ontology_dataclass_list, ontology_dataclass, "SemiRigidType",
                                                  rule_code)
        ...
        if "Kind" in ontology_dataclass.not_type and "Phase" in ontology_dataclass.not_type \
                and "Role" in ontology_dataclass.not_type and "SubKind" in ontology_dataclass.not_type:
            rule_code = "IR15"
            # LOGGER.debug(f"Executing rule {rule_code} for {ontology_dataclass.uri}.")
            m.move_classification_to_is_type_list(ontology_dataclass_list, ontology_dataclass, "NonSortal", rule_code)

        # IR17: ~Category(x) ^ ~PhaseMixin(x) ^ ~RoleMixin(x) ^ ~Mixin(x) -> Sortal(x)
        if "Category" in ontology_dataclass.not_type and "PhaseMixin" in ontology_dataclass.not_type \
                and "RoleMixin" in ontology_dataclass.not_type and "Mixin" in ontology_dataclass.not_type:
            rule_code = "IR17"
            # LOGGER.debug(f"Executing rule {rule_code} for {ontology_dataclass.uri}.")
            m.move_classification_to_is_type_list(ontology_dataclass_list, ontology_dataclass, "Sortal", rule_code)
